...
def inference():
    test_dataset = GraspNetDataset(cfgs.dataset_root, split='custom', camera=cfgs.camera, num_points=cfgs.num_point,
    ...
    for batch_idx, batch_data in enumerate(test_dataloader):
        ...
        with torch.no_grad():
            ...
            #    print(f" input batch key {k}, val {v}, {v.size()}")
            #    input()
            ...
            #for k,v in grasp_preds.items():
            #    print(f" grasp pred key {k}, val {v}")
            print(f" grasps {grasp_preds[0]}, shape {grasp_preds[0].size()} , len {len(grasp_preds)} ")
            input()
        ...
        for i in range(cfgs.batch_size):
            ...
            #input()
            gg = GraspGroup(preds)
            cloud = batch_data["point_clouds"].cpu().numpy() 
            np.save("test_custom_cloud.npy", cloud[0])
...
def vis_grasps(gg, cloud):
    ...
    print("grippers")
    print(type(grippers[0]))
    input()
    #combined = cloud + sum(grippers, start=o3d.geometry.TriangleMesh())
    #o3d.io.write_triangle_mesh("scene.glb", combined)
    #print(f" combined {combined}")
    #input()
    o3d.visualization.draw_geometries([cloud, grippers[0].to("CPU")])


def evaluate(dump_dir):
    ge = GraspNetEval(root=cfgs.dataset_root, camera=cfgs.camera, split='custom')
