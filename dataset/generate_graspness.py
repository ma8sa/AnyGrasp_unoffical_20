    ...
    for scene_id in range(90,100):
        ...
        for ann_id in range(256):
            ...
            for i in range(1, part_num + 2):   # lack of cuda memory
                ...
                #dist_, nn_inds, nn_ = knn.knn_points(grasp_points.permute(0,2,1), cloud_masked_partial.permute(0,2,1), k=1).squeeze() - 1

                #dist_,nn_inds,nn_ = knn.knn_points(grasp_points.permute(0,2,1), cloud_masked_partial.permute(0,2,1), K=1)
                dist_,nn_inds,nn_ = knn.knn_points(cloud_masked_partial.permute(0,2,1),grasp_points.permute(0,2,1),  K=1)

                nn_inds = nn_inds.squeeze()
                #pdb.set_trace()

                cloud_masked_graspness[10000 * (i - 1):(i * 10000)] = torch.index_select(grasp_points_graspness, 0, nn_inds).cpu().numpy()
