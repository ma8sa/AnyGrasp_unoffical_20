...
class GraspNetDataset(Dataset):
    def __init__(self, root, grasp_labels=None, camera='kinect', split='train', num_points=20000,
        ...
        self.root = root  # we have
        self.split = split # we have
        self.voxel_size = voxel_size # Param
        self.num_points = num_points # Param
        self.remove_outlier = remove_outlier # Param
        self.grasp_labels = grasp_labels # WE dont have, done know what to use it for 
        self.camera = camera # dont know whay it is 
        self.augment = augment # Dont know what it is 

        self.load_label = load_label # Param
    ...
    def __getitem__(self, index):
        if self.split == "custom":
            return self.get_custom_data(index) # jsut a pointcloud
        if self.load_label:
            return self.get_data_label(index)
        else:
            return self.get_data(index) # jsut a pointcloud

    def get_custom_data(self, index, return_raw_cloud=False):

        depth = np.array(Image.open(self.depthpath[index])) # depth data
        seg = np.array(Image.open(self.labelpath[index])) # segmentation mask , check what all values does it have, is bibnay for object not object or is it 
        meta = scio.loadmat(self.metapath[index]) # no fucking clue what this is ?, meta contains info about the camera 

        scene = self.scenename[index] # just the name 
        
        print(f" depth image {depth.shape}, seg image {seg.shape} ")
        input()
        #print(meta)
        ...
        #intrinsic = np.ones((3,3))# get intric directly 
        #factor_depth = 1 # choose a number later 
        ...
        print(f" got intrinsic {intrinsic}, got factor depth {factor_depth}")
        input()
        ...
        print(f"created cloud {cloud.shape}")
        input()
        # get valid points
        depth_mask = (depth > 0)
        mask = depth_mask
        ...
        print(f" depth 1 {np.sum(depth_mask)}, seg 1 {np.sum(seg_mask)}")
        print(f" num_pointws {self.num_points}")
        input()
        ...
        if len(cloud_masked) >= self.num_points:
            ...
            print(f" idxs1 {idxs1.shape}")
            print(f" idxs2 {idxs2.shape}")
